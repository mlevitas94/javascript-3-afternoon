...
//Code Here
let evenNumbers  = mixedNumbers.filter((ele) => ele % 2 ===0)
...
//Code Here
let postTaxPrices  = prices.map((ele) => ele * 1.07
 );
...
//Code Here
let totalPopulation  = populations.reduce((total, ele) =>  total+= ele)
...
let orderTotals = orders.map((item) => {
var taxed = item["tax"] + 1;
item = item["price"] * taxed;
  return item
})
...
let bobsTotal = purchases.reduce((total, item) =>{ 
  if(item["owner"] === "Bob"){
    total += item["price"];
  }
console.log(total)
